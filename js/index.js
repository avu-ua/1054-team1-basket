...
const setAgainBtn = document.querySelector('.set-again-btn')
const getBtn = document.querySelector('.get-btn')
const getAgainBtn = document.querySelector('.get-again-btn')

let clipboard

setBtn.addEventListener('click', () => {
    localStorage.setItem('myDataOne', JSON.stringify(products[0]))
})

setAgainBtn.addEventListener('click', () => {
    localStorage.setItem('myDataTwo', JSON.stringify(products[1]))
})

getBtn.addEventListener('click', () => {
    clipboard = JSON.parse(localStorage.getItem('myDataOne'))
    console.log(clipboard)
})

getAgainBtn.addEventListener('click', () => {
    clipboard = JSON.parse(localStorage.getItem('myDataTwo'))
    console.log(clipboard)
})
...
console.log('Basket  as just created: ', basket)



const buyButtonsContainer = document.querySelector('.buy_buttons_container')
buyButtonsContainer.innerHTML = ``

products.forEach(el => {
    buyButtonsContainer.innerHTML += `
    <div class = "buy-button id-${el.code}">Buy - ${el.name}</div>
    `
})
